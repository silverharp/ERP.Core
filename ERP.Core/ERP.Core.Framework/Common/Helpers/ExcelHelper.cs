...
namespace ERP.Framework.Common.Helpers
{
    ...
    public class ExcelHelper
    {
        ...
        public static string ExportDataNoConvet<T>(List<T> listData, List<ExportColumnModel> columns, T sumData,
            out string relativePath, string fileName = "", bool isSerialNumber = true,
            string dateFormat = "yyyy-MM-dd HH:mm", Dictionary<string, List<string>> rowMerge = null)
        {
            return ExportData(listData, columns, sumData, out relativePath, fileName, isSerialNumber, dateFormat,rowMerge, false);
        }
        ...
        /// <returns>绝对路径</returns>
        public static string ExportData<T>(List<T> listData, List<ExportColumnModel> columns, T sumData,
            out string relativePath, string fileName = "", bool isSerialNumber = true,
            string dateFormat = "yyyy-MM-dd HH:mm", Dictionary<string, List<string>> rowMerge = null,bool isColDatasConvert=true)
        {
            var type = typeof(T);
            ...
            //列数,包括编码要转名称的
            var columnCount = columns.Count(m => m.Datas != null && m.Datas.Count > 0 && m.Datas[0].Key.ToUpper() != "TRUE" && m.Datas[0].Key.ToUpper() != "FALSE") + columns.Count(m=>string.IsNullOrEmpty(m.ConvertKey));
            //创建头部区域
            Aspose.Cells.Range headRange = sheet.Cells.CreateRange(0, 0, 1, isSerialNumber ? columnCount + 1 : columnCount);
            headRange.SetStyle(styleTitle);
            //列数
            var colNum = 0;
            //列对应属性值及位置
            var listProp = new List<ExportColumnPropertyModel>();
            //设置列头
            foreach (var col in columns)
            {
                var point = colNum + startColumn;
                var prop = type.GetProperty(col.Key);
                //Datas有数据表示要编码转文本
                if (col.Datas == null || col.Datas.Count == 0 || !isColDatasConvert)
                {
                    cells[0, point].PutValue(col.Value);
                    listProp.Add(new ExportColumnPropertyModel
                    {
                        Key = col.Key,
                        PropertyInfo = prop,
                        Point = point
                ...
                {
                    if (col.Datas[0].Key.ToUpper() == "TRUE" || col.Datas[0].Key.ToUpper() == "FALSE")
                    {
                        cells[0, point].PutValue(col.Value);
                        listProp.Add(new ExportColumnPropertyModel
                        {
                    ...
                    else if (!string.IsNullOrWhiteSpace(col.ConvertKey))
                    {
                        prop = type.GetProperty(col.ConvertKey);
                        cells[0, point].PutValue(col.Value);
                        listProp.Add(new ExportColumnPropertyModel
                        {
                            Key = col.Key + "_Name",
                    ...
                    {
                        cells[0, point].PutValue(col.Value + "编码");
                        listProp.Add(new ExportColumnPropertyModel
                        {
                            Key = col.Key,
                            PropertyInfo = prop,
                        ...
                        cells[0, point + 1].PutValue(col.Value);
                        listProp.Add(new ExportColumnPropertyModel
                        {
                            Key = col.Key + "_Name",
                            PropertyInfo = prop,
            ...
            int rowNum = listData.Count() + headRow; //表格行数 
            ...
            for (int index = 0; index < listData.Count; index++)
            {
                ...
                foreach (var columnProp in listProp)
                {
                    SetCell(item, cells[headRow + index, columnProp.Point], columnProp, columns, dateFormat);
                }
                ...
                if (rowMerge != null)
                {
                    ...
                    var propvalue = type.GetProperty(rowValue)?.GetValue(item, null).ToString();
                    var rows = rowMeageValue.Split(',');
                    if (rows[0] != propvalue || (listData.Count == index + 1 && rows[0] == propvalue))
                    {
                        if (rows.Length == 2)
                        {
                            var totalRow = (headRow + index) - int.Parse(rows[1]);
                            if (listData.Count == index + 1 && rows[0] == propvalue)
                                totalRow = totalRow + 1;
                            if (totalRow > 1)
                            {
                                foreach (var columnName in columnNames)
                                {
                                    var colIndex = listProp.FirstOrDefault(m => m.Key == columnName)?.Point;
                                    if (colIndex != null)
                                        cells.Merge(int.Parse(rows[1]), colIndex.Value, totalRow, 1);
                                }
                            }
                        }
            ...
            Aspose.Cells.Range contentRange = sheet.Cells.CreateRange(headRow, 0, listData.Count, isSerialNumber ? columnCount + 1 : columnCount);
            ...
            if (sumData != null)
            {
                ...
                foreach (var columnProp in listProp)
                {
                    SetCell(sumData, cells[rowNum, columnProp.Point], columnProp, columns, dateFormat);
                }
                Aspose.Cells.Range footRange = sheet.Cells.CreateRange(rowNum, 0, 1, isSerialNumber ? columnCount + 1 : columnCount);
            ...
            Dictionary<string, List<string>> rowMerge = null)
        {
            ...
            Aspose.Cells.Range headRange = sheet.Cells.CreateRange(0, 0, 1,
            ...
            Aspose.Cells.Range contentRange = sheet.Cells.CreateRange(headRow, 0, listData.Rows.Count,
                isSerialNumber ? columns.Count + 1 : columns.Count);
            contentRange.SetStyle(styleContent);
            if (sumData != null)
            {
                ...
                Aspose.Cells.Range footRange = sheet.Cells.CreateRange(rowNum, 0, 1,
                    isSerialNumber ? columns.Count + 1 : columns.Count);
                footRange.SetStyle(styleFoot);
            }
            //自适应列宽
        ...
        public static string ExportDataToTableMergeColumn(DataTable listData, List<ExportColumnModel> columns,
            out string relativePath, string fileName = "", Dictionary<string, Dictionary<string, string>> rowMerge = null, bool isSerialNumber = true,
            string dateFormat = "yyyy-MM-dd HH:mm", bool boolToText = false)
        {
            if (listData == null || listData.Rows.Count == 0)
            {
            ...
            if (rowMerge != null)
            {
                var totalColNum = (rowMerge.Count * 2) + columns.Count(t => !rowMerge.Select(m => m.Key).ToList().Contains(t.Key));
                Aspose.Cells.Range headRange = sheet.Cells.CreateRange(0, 0, 2,//列为两行
                ...
                for (int index = 0; index < listData.Rows.Count; index++)
                {
                    ...
                    Aspose.Cells.Range contentRange = sheet.Cells.CreateRange(headRow, 0, listData.Rows.Count, isSerialNumber ? totalColNum + startColumn : totalColNum);
                    contentRange.SetStyle(styleContent);
                }
                //自适应列宽
                                                ...
                                                bool boolToText = false)
        {
            ...
            Aspose.Cells.Range headRange = sheet.Cells.CreateRange(0, 0, 1, isSerialNumber ? columns.Count + 1 : columns.Count);
            headRange.SetStyle(styleTitle);
            var colNum = 0;
            var colNumDic = new Dictionary<string, int>();
            foreach (var col in columns)
            ...
            if (sumData != null)
            {
                ...
                for (int i = 0; i < listProp.Count; i++)
                {
                    ...
                    { 
                        ...
                        datas[rowNum, i + 1] = value == null ? "" : "'" + value.Trim();//在obj.ToString()前加单引号是为了防止自动转化格式  
                    }
                   
                }
                Aspose.Cells.Range footRange = sheet.Cells.CreateRange(rowNum, 0, 1, isSerialNumber ? columns.Count + 1 : columns.Count);
                footRange.SetStyle(styleFoot);
            }

            Aspose.Cells.Range contentRange = sheet.Cells.CreateRange(headRow, 0, rowNum, isSerialNumber ? columns.Count + 1 : columns.Count);
            contentRange.SetStyle(styleContent);
        ...
        /// <param name="columnProp">列属性</param>
        /// <param name="columns">日期格式</param>
        /// <param name="dateFormat">日期格式</param>
        private static void SetCell<T>(T data, Cell cell, ExportColumnPropertyModel columnProp, List<ExportColumnModel> columns, string dateFormat)
        {
            var prop = columnProp.PropertyInfo;
            var type = prop.PropertyType.ToString().ToUpper();
            var value = prop.GetValue(data, null);
            if (value == null) return;
            //判断是否要转换文本
            var columnProps = columnProp.Key.Split('_');
            if (columnProps.Length == 2)
            {
                var columnModel = columns.First(m => m.Key == columnProps[0]);
                if (columnModel != null)
                {
                    if (type == "SYSTEM.BOOLEAN" || type == "SYSTEM.NULLABLE`1[SYSTEM.BOOLEAN]")
                    {
                        var columnDatas = columnModel.Datas.Where(m => Convert.ToBoolean(m.Key) == Convert.ToBoolean(value)).ToList();
                        value = columnDatas.Count>0 ? columnDatas[0].Value : "";
                    }
                    else
                    {
                        var columnDatas = columnModel.Datas.Where(m => m.Key == Convert.ToString(value)).ToList();
                        value = columnDatas.Count>0 ? columnDatas[0].Value : "";
                    }
                }
            ...
            else if (type == "SYSTEM.DECIMAL" || type == "SYSTEM.NULLABLE`1[SYSTEM.DECIMAL]")
            {
                try
                {
                    ...
                    cell.PutValue(Convert.ToDecimal(value.ToString()).ToString("0.".PadRight(vs[1].Length + 2, '0')));
                }
                catch
                {
                    cell.PutValue($"{value:F}");
                }
        ...
        /// <returns>数据集</returns>
        public static List<T> ImportData<T>(string excelfile, Dictionary<string, string> columDictionary, int firstRow = 0, int firstColumn = 0) where T : new()
        {
            Workbook workbook = new Workbook(excelfile);
            Cells cells = workbook.Worksheets[0].Cells;
            DataTable dt = cells.ExportDataTableAsString(firstRow, firstColumn, cells.MaxDataRow + 1, cells.MaxColumn + 1, true);
            RemoveEmpty(dt);
            if (dt == null || dt.Rows.Count == 0)
            { 
                throw new Exception("没有数据导入");
            }

            PropertyInfo[] properties = typeof(T).GetProperties();//获取实体类型的属性集合 

            var proDictionary = properties.Where(pro => columDictionary.ContainsKey(pro.Name)).ToDictionary(pro => pro.Name);
        ...
        /// <returns></returns>
        public static List<T> ImportData<T>(out string errorMessage, string excelfile, List<ImportColumnModel> columDictionary, string sheetName = "数据模板", int firstRow = 0, int firstColumn = 0) where T : new()
        {
            errorMessage = string.Empty;
            Workbook workbook = new Workbook(excelfile);
            ...
            if (woksheets == null)
            {
                throw new Exception(string.Format("未找到工作簿",sheetName));
            } 
        ...
        /// <returns></returns>
        private static List<T> GetImportData<T>(ref string errorMessage, List<ImportColumnModel> columDictionary, int firstRow, int firstColumn,
            Cells cells) where T : new()
        {
            ...
            var proDictionary =
                properties.Where(pro => columDictionary.Any(t => t.PropertyName == pro.Name)).ToDictionary(pro => pro.Name);
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                foreach (var proItem in proDictionary)
                {
                    ...
                    if (dataValue != null && columnItem.DataSource != null && Convert.ToString(dataValue).Trim() != "" && !columnItem.DataSource.Contains(dataValue))
                    {
            ...
            if (allEmptyBuilder.Count > 0)
            {
                errorMessage += allEmptyBuilder.Aggregate(string.Empty,
                    (current, pair) => current + string.Format("Excel不能为空" + "\r\n", pair.Key, pair.Value.Length > 50 ? pair.Value.ToString().Substring(0, 50) + "..." : pair.Value.ToString()));
            }
            ...
            if (allExitBuilder.Count > 0)
            {
                ...
                foreach (var pair in allExitBuilder)
                {
                    ...
                    result = result + string.Format(item + "；\r\n", pair.Value.Length > 50 ? pair.Value.ToString().Substring(0, 50) + "..." : pair.Value.ToString());
                }
                errorMessage += result;
            }
        ...
        private static void AddError(Dictionary<string, StringBuilder> messageBuilder, string key, object objString, string separate = "")
        {
